...
export default class Grid implements SimpleGameSubject {
  ...
  public connect(eventBus: EventBus) {
    eventBus.get<DOMEventArgs<KeyboardEvent>>('keydown').subscribe({
      next: args => this.act(args.native.key)
    })
    eventBus.get<RedrawEventArgs>('redraw').subscribe({
      next: args => this.draw(args)
    })
    eventBus.get<UpdateEventArgs>('update').subscribe({
      next: _ => this.update(eventBus)
    })
  ...
  protected update(eventBus: EventBus) {
    if (this._lost) {
